...
struct TrackInformation{
	...
	double computeTOFinSuperFGD() /*ns*/
	{
		...
		for (int ipt = 0; ipt < NPoints; ipt++)
		{
			...
			if (ipt == 0) start_time = point.PtTime;
			stop_time = point.PtTime;
		}
	...
	std::tuple<std::string, std::string, std::string, std::string> asSvgIntoSuperFGD()
	{
		...
		/// new variant
		std::string prev_pt_xz, prev_pt_yz, prev_pt_xy;
		std::string short_pt_info_xz = "", short_pt_info_yz = "", short_pt_info_xy = "";
		PointInformation prev_point;
		for (int ipt = 0; ipt < NPoints; ipt++)
		{
			PointInformation point(nd280UpTrack, ipt, PDG);
			if (!point.isIntoSuperFGD()) continue;

			std::string sPtPos_xz = std::to_string(int(point.PtPostPos[0])) + "," + std::to_string(int(point.PtPostPos[2]));
			std::string sPtPos_yz = std::to_string(int(point.PtPostPos[1])) + "," + std::to_string(int(point.PtPostPos[2]));
			std::string sPtPos_xy = std::to_string(int(point.PtPostPos[0])) + "," + std::to_string(int(point.PtPostPos[1]));

			if (sPtPos_xz == prev_pt_xz && sPtPos_yz == prev_pt_yz && sPtPos_xy == prev_pt_xy) continue;

			short_pt_info_xz += sPtPos_xz + ":" + point.shortInfo(prev_point, PDG) + ";";
			short_pt_info_yz += sPtPos_yz + ":" + point.shortInfo(prev_point, PDG) + ";";
			short_pt_info_xy += sPtPos_xy + ":" + point.shortInfo(prev_point, PDG) + ";";
			...
			prev_pt_xz = sPtPos_xz;
			prev_pt_yz = sPtPos_yz;
			prev_pt_xy = sPtPos_xy;

			prev_point = point;
		}

		/// remove last character (;)
		if (short_pt_info_xz.size()) short_pt_info_xz.pop_back();
		if (short_pt_info_yz.size()) short_pt_info_yz.pop_back();
		if (short_pt_info_xy.size()) short_pt_info_xy.pop_back();
		...
		std::stringstream formattedEdep;
		formattedEdep << std::setprecision(7) << SumEdepAllPts;

		std::stringstream formattedDirectionX, formattedDirectionY, formattedDirectionZ;
		// TVector3 direction = getDirection();
		formattedDirectionX << std::setprecision(7) << Direction.X();
		formattedDirectionY << std::setprecision(7) << Direction.Y();
		formattedDirectionZ << std::setprecision(7) << Direction.Z();
		...
		// XMLObject * svgTrackTitle = new XMLObject("title",
		// 	"TrkId:" + std::to_string(TrackID) +\
		// 	", ParId:" + std::to_string(ParentID) +\
		// 	", PDG:" + std::to_string(PDG) +\
		// 	", CreatorProc:" + ProcessName +\
		...
		XMLObject * svgTrackTitle_xz = new XMLObject("title",
			"TrkId:" + std::to_string(TrackID) +\
			", ParId:" + std::to_string(ParentID) +\
			", PDG:" + std::to_string(PDG) +\
			", CreatorProc:" + ProcessName +\
		...
		XMLObject * svgTrackTitle_yz = new XMLObject("title",
			"TrkId:" + std::to_string(TrackID) +\
			", ParId:" + std::to_string(ParentID) +\
			", PDG:" + std::to_string(PDG) +\
			", CreatorProc:" + ProcessName +\
		...
		XMLObject * svgTrackTitle_xy = new XMLObject("title",
			"TrkId:" + std::to_string(TrackID) +\
			", ParId:" + std::to_string(ParentID) +\
			", PDG:" + std::to_string(PDG) +\
			", CreatorProc:" + ProcessName +\
		...
		XMLObject * svgTrackPath_xz = new XMLObject("path", "", {
			{"id", std::to_string(TrackID)},
			{"data-parentid", std::to_string(ParentID)},
			{"data-trklen", formattedTrkLen.str()},
			{"data-trkE0", formattedTrkInitKinEnergy.str()},
			{"data-edep", formattedEdep.str()},
			{"data-dirX", formattedDirectionX.str()},
			{"data-dirY", formattedDirectionY.str()},
			{"data-dirZ", formattedDirectionZ.str()},
			{"data-pdg", std::to_string(PDG)},
			{"data-npts", std::to_string(NPoints)},
			{"data-creatorproc", ProcessName},
			{"class", "trackpath pdg" + std::to_string(PDG) + " id" + std::to_string(TrackID)},
			{"fill", "none"}, {"stroke", color()}, {"stroke-width", "2"},
			{"d", path_xz},
			{"data-ptinfo", short_pt_info_xz}
		}); 
		svgTrackPath_xz->appendChild(svgTrackTitle_xz);
		// svgTrackPath_xz->appendChild(svgTrackTitle);

		XMLObject * svgTrackPath_yz = new XMLObject("path", "", {
			{"id", std::to_string(TrackID)},
			{"data-parentid", std::to_string(ParentID)},
			{"data-trklen", formattedTrkLen.str()},
			{"data-trkE0", formattedTrkInitKinEnergy.str()},
			...
			{"fill", "none"}, {"stroke", color()}, {"stroke-width", "2"},
			{"d", path_yz},
			{"data-ptinfo", short_pt_info_yz}
		}); 
		svgTrackPath_yz->appendChild(svgTrackTitle_yz);
		// svgTrackPath_yz->appendChild(svgTrackTitle);

		XMLObject * svgTrackPath_xy = new XMLObject("path", "", {
			{"id", std::to_string(TrackID)},
			{"data-parentid", std::to_string(ParentID)},
			{"data-trklen", formattedTrkLen.str()},
			{"data-trkE0", formattedTrkInitKinEnergy.str()},
			...
			{"fill", "none"}, {"stroke", color()}, {"stroke-width", "2"},
			{"d", path_xy},
			{"data-ptinfo", short_pt_info_xy}
		}); 
		svgTrackPath_xy->appendChild(svgTrackTitle_xy);
		// svgTrackPath_xy->appendChild(svgTrackTitle);

		path_xz = svgTrackPath_xz->to_str(0, 0);
		path_yz = svgTrackPath_yz->to_str(0, 0);
		path_xy = svgTrackPath_xy->to_str(0, 0);

		svgTrackPath_xz->clear();
		svgTrackPath_yz->clear();
		svgTrackPath_xy->clear();
